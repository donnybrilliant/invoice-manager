...
/**
 * Internal function: Prepares element and captures it as canvas
 * Shared logic for both download and base64 PDF generation
 */
async function prepareElementAndCaptureCanvas(
  element: HTMLElement
...
/**
 * Internal function: Creates PDF with smart scaling from canvas
 * Shared logic for both download and base64 PDF generation
 */
function createPDFWithSmartScaling(
...
export async function generatePDFFromElement(
  ...
  try {
    const canvas = await prepareElementAndCaptureCanvas(element);
    const pdf = createPDFWithSmartScaling(canvas, config, false);
...
export async function generatePDFBase64(
  ...
  const canvas = await prepareElementAndCaptureCanvas(element);
  const pdf = createPDFWithSmartScaling(canvas, config, true);
