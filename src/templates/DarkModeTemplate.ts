    ...
    const itemsHtml = items
      .map(
        ...
        <tr style="background: ${
          ...
          <td style="padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.1); color: #E0E0E0; font-size: 14px;">${
            item.description
          }</td>
          <td style="padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.1); text-align: center; color: #A0A0A0;">${
            item.quantity
          }</td>
          <td style="padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.1); text-align: right; color: #A0A0A0;">${formatCurrencyWithCode(
            item.unit_price,
            invoice.currency
          )}</td>
          <td style="padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.1); text-align: right; color: #00FFB2; font-weight: 600;">${formatCurrencyWithCode(
            item.amount,
            invoice.currency
          )}</td>
        </tr>
      ...
      <div style="font-family: 'SF Pro Display', -apple-system, BlinkMacSystemFont, sans-serif; max-width: 800px; margin: 0 auto; padding: 50px; background: linear-gradient(180deg, #0D0D0D 0%, #1A1A1A 100%); color: #fff; border-radius: 12px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 50px; padding-bottom: 30px; border-bottom: 1px solid rgba(255,255,255,0.1);">
          <div>
            ${
              profile?.logo_url
                ? `<img src="${profile.logo_url}" alt="Logo" style="max-width: 140px; max-height: 70px; margin-bottom: 20px; filter: brightness(0) invert(1);" />`
                : ""
            }
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 50px;">
          <div style="padding: 25px; background: rgba(0,255,178,0.05); border-radius: 8px; border: 1px solid rgba(0,255,178,0.2);">
            ...
            <div style="font-size: 18px; font-weight: 600; color: #fff; margin-bottom: 10px;">${
              client.name
            }</div>
            <div style="font-size: 13px; color: #808080; line-height: 1.8;">
          ...
          <div style="padding: 25px; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.08);">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 25px;">
              <div>
                ...
                <div style="font-size: 15px; color: #E0E0E0;">${formatDate(
                  invoice.issue_date
                )}</div>
              </div>
              <div>
                ...
                <div style="font-size: 15px; color: #FF6B6B;">${formatDate(
                  invoice.due_date
                )}</div>
              </div>
            </div>
        ...
        <div style="display: flex; justify-content: flex-end; margin-bottom: 50px;">
          <div style="width: 320px; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px solid rgba(255,255,255,0.08); overflow: hidden;">
            <div style="display: flex; justify-content: space-between; padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.08);">
              ...
              <span style="font-size: 14px; color: #E0E0E0;">${formatCurrencyWithCode(
                invoice.subtotal,
                invoice.currency
              )}</span>
            </div>
            ...
            <div style="display: flex; justify-content: space-between; padding: 20px; background: linear-gradient(135deg, rgba(0,255,178,0.15) 0%, rgba(0,255,178,0.05) 100%);">
              ...
              <span style="font-size: 28px; font-weight: 300; color: #00FFB2;">${formatCurrencyWithCode(
                invoice.total,
                invoice.currency
              )}</span>
            </div>
