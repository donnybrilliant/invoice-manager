  ...
  render: (data: InvoiceTemplateData): string => {
    ...
    const itemsHtml = items
      .map(
        (item) => `
        <tr>
          <td style="padding: 12px 16px; border: 3px solid #000; font-family: 'Courier New', monospace; font-size: 13px; text-transform: uppercase;">${
            item.description
          }</td>
          <td style="padding: 12px 16px; border: 3px solid #000; text-align: center; font-family: 'Courier New', monospace; font-weight: 900;">${
            item.quantity
          }</td>
          <td style="padding: 12px 16px; border: 3px solid #000; text-align: right; font-family: 'Courier New', monospace;">${formatCurrencyWithCode(
            item.unit_price,
            invoice.currency
          )}</td>
          <td style="padding: 12px 16px; border: 3px solid #000; text-align: right; font-family: 'Courier New', monospace; font-weight: 900; background: #000; color: #fff;">${formatCurrencyWithCode(
            item.amount,
            invoice.currency
          )}</td>
        </tr>
      ...
      <div style="font-family: 'Courier New', Courier, monospace; max-width: 800px; margin: 0 auto; padding: 40px; background: #fff; border: 6px solid #000;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 40px; padding-bottom: 30px; border-bottom: 6px solid #000;">
          ...
          <div style="text-align: right; border: 3px solid #000; padding: 20px; max-width: 280px;">
            ${
              profile?.logo_url
                ? `<img src="${profile.logo_url}" alt="Logo" style="max-width: 120px; max-height: 60px; margin-bottom: 15px; filter: grayscale(100%);" />`
                : ""
            }
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0; margin-bottom: 40px; border: 3px solid #000;">
          <div style="padding: 20px; border-right: 3px solid #000;">
            ...
            <div style="font-size: 18px; font-weight: 900; text-transform: uppercase; margin-bottom: 8px;">${
              client.name
            }</div>
          ...
          <div style="padding: 20px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
              <div>
                ...
                <div style="font-size: 14px; font-weight: 900; margin-top: 4px;">${formatDate(
                  invoice.issue_date
                )}</div>
              </div>
              <div>
        ...
        <div style="display: flex; justify-content: flex-end; margin-bottom: 40px;">
          <div style="width: 320px; border: 6px solid #000;">
            ...
            <div style="display: flex; justify-content: space-between; padding: 20px 16px; background: #000; color: #fff;">
              ...
              <span style="font-size: 24px; font-weight: 900;">${formatCurrencyWithCode(
                invoice.total,
                invoice.currency
              )}</span>
            </div>
